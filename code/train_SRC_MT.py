...
from networks.models import DenseNet169
...
parser.add_argument('--num_classes', type=float,  default=15, help='num_classes')
...
if __name__ == "__main__":
    ...
    def create_model(ema=False):
        # Network definition
        net = DenseNet169(out_size=chest_xray_14.N_CLASSES, mode=args.label_uncertainty, drop_rate=args.drop_rate)
    ...
    unlabeled_idxs = list(range(args.labeled_num, 80232))
    batch_sampler = chest_xray_14.TwoStreamBatchSampler(labeled_idxs, unlabeled_idxs, batch_size,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (_, _, (image_batch, ema_image_batch), label_batch) in enumerate(train_dataloader):
            ...
            if args.ema_consistency == 1:
                ...
                consistency_dist = torch.sum(
                    losses.softmax_mse_loss(outputs, ema_output)) / batch_size  # / dataset.N_CLASSES
                ...
                consistency_relation_dist = torch.sum(
                    losses.relation_mse_loss(activations, ema_activations)) / batch_size
        ...
        AUROCs = epochVal(model, val_dataloader, loss_fn, args)
        ...
        logging.info("\nVAL AUROC: {:6f}"
                     .format(AUROC_avg))
        logging.info(
            "AUROCs: " + " ".join(["{}:{:.6f}".format(chest_xray_14.CLASS_NAMES[i], v) for i, v in enumerate(AUROCs)]))

        # test student
        #
        AUROCs = epochVal(model, test_dataloader, loss_fn, args)
        AUROC_avg = np.array(AUROCs).mean()
